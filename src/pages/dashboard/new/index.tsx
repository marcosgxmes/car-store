...
const schema = z.object({
  ...
  model: z.string().nonempty("O campo modelo é obirgatório *"),
...
export function New() {
  ...
  function onSubmit(data: FormData) {
    if (carImages.length === 0) {
      ...
      toast.error("É necessário enviar pelo menos uma imagem")
      return;
    }
  ...
  async function handleFile(e: ChangeEvent<HTMLInputElement>) {
    if (e.target.files && e.target.files[0]) {
      ...
      if (image.type === 'image/jpeg' || image.type === 'image/png') {
        ...
        alert("Selecione uma arquivo no formato jpeg ou png!")
        return;
      }
    }
  }
  ...
  return (
    <Container>
      ...
      <div className="w-full bg-white p-3 rounded-lg flex flex-col sm:flex-row items-center gap-2">
        ...
        {carImages.map( item => (
          <div key={item.name} className="w-full h-32 flex items-center justify-center relative">
            ...
            <img src={item.previewUrl} alt="foto do carro" className="roundedn-lg w-full h-32 object-cover" />
          </div>
        ))}
      </div>
      ...
      <div className="w-full bg-white p-3 rounded-lg flex flex-col sm:flex-row items-center gap-2 mt-2">
        ...
        >
          <div className="mb-3 ">
            <p className="mb-3 font-medium">Nome do carro</p>
            <Input
              type="text"
              register={register}
              name="name"
              error={errors.name?.message}
              placeholder="Ex: GOL G3 1.6..." />
          </div>

          <div className="mb-3 ">
            <p className="mb-3 font-medium">Modelo</p>
            <Input
              type="text"
              register={register}
              name="model"
              error={errors.model?.message}
              placeholder="Ex: 1.6 flex 8v" />
          </div>

          <div className="flex w-full mb-3 flex-row items-center gap-4 ">
            <div className="w-full">
              <p className="mb-3 font-medium">Ano</p>
              <Input
                type="text"
                register={register}
                name="year"
                error={errors.year?.message}
                placeholder="Ex: 2002/2003..." />
            </div>

            <div className="w-full">
              <p className="mb-3 font-medium">Km</p>
              <Input
                type="text"
                register={register}
                name="km"
                error={errors.km?.message}
                placeholder="Ex: 56.000Km..." />
            </div>
          </div>
          ...
          <div className="mb-3 ">
            <p className="mb-3 font-medium">Preço</p>
            <Input
              type="text"
              register={register}
              name="price"
              error={errors.price?.message}
              placeholder="Ex: R$ 16.000" />
          </div>

          <div className="mb-3 ">
            <p className="mb-3 font-medium">Descrição</p>
            <textarea
